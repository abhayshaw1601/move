...
function renderAuditReportHtml(input) {
    ...
    const rowsHtml = versions
        ...
        const metricsCells = Object.entries(v.metrics)
            ...
            .join("");
        return `<tr><td>${v.versionId}</td><td>${new Date(v.timestampMs).toISOString()}</td><td>${v.message}</td>${metricsCells}</tr>`;
    })
        .join("\n");
    const diffSections = versions
        .map((v) => (0, diff_1.renderDiffHtml)(v.oldText, v.newText, v.fileName))
        .join("\n");
    return `<!DOCTYPE html>
<html lang="en">
  <head>
      ...
      <table>
        <thead>
          <tr>
            ...
            <th>Timestamp</th>
            <th>Message</th>
            <th colspan="${Object.keys(versions[0]?.metrics ?? { Accuracy: "" }).length * 2}">Metrics</th>
          </tr>
        </thead>
        <tbody>
${rowsHtml}
